<x-guest-layout>
    <form method="POST" action="{{ route('register') }}">
        ...
        <div class="flex justify-end items-center pt-5 gap-3">
            <div >
                <a class="underline text-sm text-[#feda97]  transition duration-300" href="{{ route('login') }}">
                    {{ __('Already registered?') }}
                </a>
            </div>

            <div >
                <x-primary-button class="px-5 py-3 mx-auto text-white rounded-full bg-black">
                    {{ __('Register') }}
                </x-primary-button>
            </div>
        </div>
