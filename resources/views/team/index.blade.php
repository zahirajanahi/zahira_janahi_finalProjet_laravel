...
<div class="relative h-[27vh] w-[94.6vw] bg-black ml-[5.4vw]">
    <!-- Image -->
    <img src="{{ asset('storage/images/createteam.png') }}" class="h-full w-full object-cover opacity-80" alt="">
  ...
  <div class="">
    <button onclick="document.getElementById('createTaskModal').classList.remove('hidden');" class="ms-40 mt-5 border-[#ba3819] ps-2 border-s-2 h-[5vh] pt-1 px-4  py-2  text-gray-600">Create Team</button>
  </div>
  ...
  <div id="createTaskModal" class="fixed inset-0 bg-gray-500 bg-opacity-30 flex items-center justify-center z-50 hidden transition-opacity duration-300">
    ...
    <div class="bg-white border border-gray-300 rounded-2xl shadow-lg w-full max-w-md p-6">
        ...
        <form action="{{ route('team.store') }}" method="POST" class="mt-4">
            ...
            <!-- Task description -->
            <div class="mb-4">
                <label for="description" class="block text-gray-600 text-sm font-medium mb-1">Team Description</label>
                <textarea
                    name="description"
                    id="description"
                    rows="2"
...
<div class="flex flex-wrap gap-3 ms-40">
    ...
    <div 
        class="w-[30vw] mt-10 bg-gray-100 p-6 rounded-3xl h-[30vh] cursor-pointer relative"
        onclick="showSidebar('{{ $team->name }}', '{{ $team->description }}')">
        <div class="justify-between flex">
            <h1 class="text-black font-bold text-xl">{{ $team->name }}</h1>
            <button>
                <p class="text-[#6b0c02]">join</p>
            </button>
        </div>
        ...
        <div class="flex items-center absolute bottom-3 left-5 right-0 ">
            ...
            <div class="rounded-full p-2 z-30">
                <img
                    src="{{ asset('storage/images/user-4.png') }}"
                    alt="Person 1"
            ...
            <div class="-ml-6 rounded-full  p-2 z-20">
                <img
                src="{{ asset('storage/images/user-8.png') }}"
                alt="Person 2"
                    class="rounded-full w-7 h-7 object-cover"
                />
            </div>
            ...
            <div class="-ml-6 rounded-full  p-2 z-10">
                <img
                src="{{ asset('storage/images/user-6.png') }}"
                alt="Person 3"
                    class="rounded-full w-7 h-7 object-cover"
                />
            </div>
            <div class="text-sm text-gray-500 pb-1 ps-1">+ more</div>
        </div>
    </div>
    @endforeach
</div>
...
<!-- Team show more -->
<div id="team-sidebar" class="fixed top-0 right-0 w-[50vw] h-full bg-gray-100 shadow-2xl p-6 z-50 mt-20 transform translate-x-full transition-transform duration-500 rounded-3xl">
    <button class="text-[#6b0c02] float-right" onclick="hideSidebar()">x</button>
    <div class="flex justify-between items-center">
        <h2 id="sidebar-title" class="text-xl font-bold"></h2>
       <button  onclick="event.stopPropagation(); document.getElementById('invite').classList.remove('hidden');"><i class="bi bi-person-plus text-lg font-bold  me-10 text-gray-500"  ></i></button> 
    </div>
    <h2 id="sidebar-title" class="text-xl font-bold"></h2>
 ...
 {{-- invite modal --}}
<div id="invite" class="fixed inset-0 bg-gray-500 bg-opacity-30 flex items-center justify-center z-50 hidden transition-opacity duration-300">
    <!-- Modal content -->
    <div class="bg-white border border-gray-300 rounded-2xl shadow-lg w-full max-w-md p-6">
        <!-- Modal header -->
        <div class="flex justify-between items-center pb-4 border-b border-gray-200">
            ...
            <button
                class="text-gray-400 hover:text-gray-600 text-2xl focus:outline-none cursor-pointer transition duration-200"
                onclick="document.getElementById('invite').classList.add('hidden');"
            >
                &times;
            </button>
        </div>
        ...
        <form action="" method="POST" class="mt-4">
            ...
            <!-- Task name -->
            <div class="mb-4">
                <label for="name" class="block text-gray-600 text-sm font-medium mb-1">Invite by email</label>
                <input
                    id="email"
                    type="email"
                    name="email"
                    class="block w-full px-4 py-2 text-gray-800 bg-gray-100 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500 transition duration-200"
                    placeholder="Add members to join your team"
                    required
                />
            </div>
